 ...
 * Теперь напиши адрес откуда ты хочешь уехать
 *
 ...
 *
 * После чего нужно переводить юзера на следующий этап [[StageType.CreateTripSetSecond]]  и выводить его [[sendFirstMessage]]
 *
 * @author Egor
 */
object CreateTripSetFirst extends Stage {

  override val stageType: StageType = StageType.CreateTripSetFirst
  ...
  override def messageReceiveProcess(receive: MessageReceive)(implicit ec: ExecutionContext): Future[Unit] = {
    for {
      ...
      isAlreadyDefined = tripPoints.exists {
        _.tripPointType == TripPointType.starting
      }
      _ <- if (isAlreadyDefined)
        ...
        val point = TripPoint(None, trip.id.get, receive.text, None, None, TripPointType.starting)
        for {
